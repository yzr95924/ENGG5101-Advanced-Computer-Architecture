...
{	
  ...
  {
    case OP_HALT:
      if(LOG) printf("Instruction: halt\n");
      exit(0);
      break;
    case OP_NOP:
      if(LOG) printf("Instruction: NOP\n");
      cp->cpu.PC ++;
      break;
    case OP_ADDI:
      if(LOG) printf("Instruction: addi R%d,R%d,%d\n", *p_sreg, *p_treg, *p_imm);
      cp->cpu.R[*p_treg] = cp->cpu.R[*p_sreg] + *p_imm;
      cp->cpu.PC ++;
      break;
    case OP_MOVEREG:
      if(LOG) printf("Instruction: move_reg R%d,R%d\n", *p_sreg, *p_treg);
      cp->cpu.R[*p_treg] = cp->cpu.R[*p_sreg];
      cp->cpu.PC ++;
      break;
    case OP_MOVEI:
      if(LOG) printf("Instruction: movei R%d,%d\n", *p_treg, *p_imm);
      cp->cpu.R[*p_treg] = *p_imm;
      cp->cpu.PC ++;
      break;
    case OP_LW:
      if(LOG) printf("Instruction: lw R%d,R%d,%d\n", *p_sreg, *p_treg, *p_imm);
      cp->cpu.R[*p_treg] = cp->memory.addr[(cp->cpu.R[*p_sreg] + *p_imm)];
      cp->cpu.PC ++;
      break;
    case OP_SW:
      if(LOG) printf("Instruction: sw R%d,R%d,%d\n", *p_sreg, *p_treg, *p_imm);
      cp->memory.addr[(cp->cpu.R[*p_sreg] + *p_imm)] = cp->cpu.R[*p_treg];
      cp->cpu.PC ++;
      break;
    case OP_BLEZ:
      if(LOG) printf("Instruction: blez R%d,%d\n", *p_sreg, *p_imm);
      ...
      break;
    case OP_LA:
      if(LOG) printf("Instruction: la R%d,%d\n", *p_treg, *p_imm);
      cp->cpu.R[*p_treg] = cp->cpu.PC + 1 + *p_imm;
      cp->cpu.PC ++;
      break;
    case OP_ADD:
      if(LOG) printf("Instruction: add R%d,R%d\n", *p_sreg, *p_treg);
      cp->cpu.R[*p_treg] = cp->cpu.R[*p_treg] + cp->cpu.R[*p_sreg];
      cp->cpu.PC ++;
      break;
    case OP_JMP:
      if(LOG) printf("Instruction: jmp %d\n", *p_imm);
      cp->cpu.PC = cp->cpu.PC + 1 + *p_imm;
      break;
    case OP_PUSH:
      if(LOG) printf("Instruction: push R%d\n", *p_sreg);
      cp->cpu.SP = cp->cpu.SP - 1;
      cp->memory.addr[cp->cpu.SP] = cp->cpu.R[*p_sreg];
      cp->cpu.PC ++;
      break;
    case OP_POP:
      if(LOG) printf("Instruction: pop R%d\n", *p_treg);
      cp->cpu.R[*p_treg] = cp->memory.addr[cp->cpu.SP];
      cp->cpu.SP = cp->cpu.SP + 1;
      cp->cpu.PC ++;
      break;
    case OP_IRET:
      if(LOG) printf("Instruction: iret\n");
      // PC <- Pop()
      ...
      break;
    case OP_PUT:
      if(LOG) printf("Instruction: put R%d\n", *p_sreg);
...
int timer_tick(COMPUTER* cp)
{
  ...
  if(cp->cpu.counter % 5000 == 0){
    ...
    if((cp->cpu.PSR & PSR_INT_EN) == PSR_INT_EN){
      ...
      if(LOG) printf("Enter the timer_tick\n");
    }
  }
	return 0;
}
...
int check_interrupt(COMPUTER* cp)
{
  ...
  if(((cp->cpu.PSR & PSR_INT_EN) == PSR_INT_EN) && 
      ...
      if(LOG) printf("Enter the check_interrupt!\n");
      cp->cpu.SP = cp->cpu.SP - 1;
      cp->memory.addr[cp->cpu.SP] = cp->cpu.PSR;
      cp->cpu.SP = cp->cpu.SP - 1;
      cp->memory.addr[cp->cpu.SP] = cp->cpu.PC;
